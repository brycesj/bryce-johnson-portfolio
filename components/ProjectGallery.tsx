...
export function ProjectGallery({ media, columns = "auto", variant = "default" }: ProjectGalleryProps) {
  ...
  return (
    <div className={`grid gap-5 ${gridClass}`}>
      {media.map((item, index) => {
        ...
        if (item.type === "video") {
          return (
            <div className={variant === "showcase" ? "mx-auto w-full max-w-5xl" : "md:col-span-2 lg:col-span-full"} key={`${item.src}-${item.caption}`}>
              <VideoEmbed src={item.src} caption={item.caption} />
            </div>
          );
        }

        const imageFit = item.fit === "contain" ? "object-contain" : "object-cover";
        ...
        const aspectClass =
          item.aspect === "portrait"
            ...
            : item.aspect === "wide" || item.fit === "contain"
              ...
              : "aspect-[4/3]";
        const figureClass =
          variant === "showcase" && item.aspect === "portrait"
            ? "mx-auto max-w-2xl"
            : variant === "showcase"
              ? "mx-auto max-w-5xl"
              : "";
        ...
        const image = (
          <figure className={`group h-full overflow-hidden rounded-2xl border border-cyan-400/20 bg-slate-900/80 shadow-soft transition duration-300 hover:-translate-y-1 hover:border-cyan-300/50 hover:shadow-lift ${figureClass}`}>
